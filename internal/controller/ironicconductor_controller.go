...
func (r *IronicConductorReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager) error {
	...
	if err := mgr.GetFieldIndexer().IndexField(ctx, &ironicv1.IronicConductor{}, transportURLSecretField, func(rawObj client.Object) []string {
		// Extract the secret name from the spec, if one is provided
		cr := rawObj.(*ironicv1.IronicConductor)
	...
	cms := []util.Template{
		// Scripts ConfigMap
		{
			...
			AdditionalTemplate: map[string]string{
				...
				"live_check_conductor": "/ironicconductor/bin/live_check_conductor",
			},
			Labels: cmLabels,
		},
		// Custom ConfigMap
